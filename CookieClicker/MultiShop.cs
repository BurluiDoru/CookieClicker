...
namespace CookieClicker
{
    public partial class MultiShop : Form
    {
        ...
        private void multi2x_Click(object sender, EventArgs e)
        {
            ...
            bool succes = Form1.Instanta.CumparaUpgradeInteligent(CatalogUpgradeuri.listaMulti[0]);
        ...
        private void multi5x_Click(object sender, EventArgs e)
        {
            bool succes = Form1.Instanta.CumparaUpgradeInteligent(CatalogUpgradeuri.listaMulti[1]);
            if (succes) ActualizeazaPreturi();
        }

        // Butonul pentru al treilea upgrade (Multi 10x) -> Indexul 2
        private void multi10x_Click(object sender, EventArgs e)
        {
            bool succes = Form1.Instanta.CumparaUpgradeInteligent(CatalogUpgradeuri.listaMulti[2]);
            if (succes) ActualizeazaPreturi();
        }

        // Butonul pentru al patrulea upgrade (Multi 25x) -> Indexul 3
        private void multi25x_Click(object sender, EventArgs e)
        {
            bool succes = Form1.Instanta.CumparaUpgradeInteligent(CatalogUpgradeuri.listaMulti[3]);
            if (succes) ActualizeazaPreturi();
        }
    }
}